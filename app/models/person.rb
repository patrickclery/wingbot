class Person < ApplicationRecord
  ...
  def self.from_recommendation(recommendation)
    p = find_or_initialize_by(tinder_id: recommendation.user._id)
    p.assign_attributes bio:           recommendation.user.bio,
  ...
  def self.from_person(person_struct)
    p = find_or_initialize_by(tinder_id: person_struct._id)
    p.assign_attributes bio:       person_struct.bio,
